var bijiDeck = (function(){
	...
	function bijiDeck(people_num, $c, face){
		...
		deck.setAllLeft = setAllLeft;
		deck.addPerson = addPerson;
		deck.setPeopleDetails = setPeopleDetails
		...
		function setAllLeft(pointsMap){
			for(var i=0;i<people_detail.length;i++){
				var poitLeft = pointsMap[people_detail[i].person_id]
				people_detail[i].total_left = poitLeft
				people_detail[i].$span2.innerHTML = poitLeft;
			}
		}
		...
		function setPeopleDetails(players) {
			for (var i=0;i<players.length;i++) {
				people_detail[i].person_id = players[i].userId
				people_detail[i].nickName = players[i].nickName
				people_detail[i].$span1.innerHTML = players[i].nickName;
				people_detail[i].$people.style.display = 'block';
			}
		}
	}
